  ...
  if (fs.existsSync(TOKEN_PATH)) {
    ...
    try {
      ...
      if (parsedToken.refresh_token) {
        ...
        logError("Refresh token is missing from stored credentials so going by requesting new token..");
    ...
    if (req.url.indexOf("/?code=") > -1) {
      ...
      oAuth2Client.getToken(code, (err, token) => {
        ...
        if (!token.refresh_token) {
          logError("Refresh token is missing in the response but still storing the token.");
